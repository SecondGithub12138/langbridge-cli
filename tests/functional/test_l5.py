...
def _use_tmp_plans(monkeypatch, tmp_path):
    monkeypatch.setattr("langbridge_cli.component_plan.COMPONENT_PLAN_DIR", tmp_path)
...
def test_l5_delivers_when_every_sub_task_passes(tmp_path, monkeypatch):
    ...
    def fake_l5(api_key, model, task, context, feedback):
        calls["l5"] += 1
    ...
    monkeypatch.setattr("langbridge_cli.multi_agent.run_l5_engineer", fake_l5)
...
def test_l5_push_back_goes_to_jury_and_passes(tmp_path, monkeypatch):
    ...
    assert "PM_REVIEW_STATUS: OK" in output
    worklog = (tmp_path / "run.worklog.md").read_text(encoding="utf-8")
